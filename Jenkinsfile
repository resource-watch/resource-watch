  ...
  try {
    ...
    stage ('Build docker') {
      switch ("${env.BRANCH_NAME}") {
        case "develop":
          sh("docker -H :2375 build --build-arg secretKey=${secretKey} --build-arg NEXT_PUBLIC_RW_GOGGLE_API_TOKEN_SHORTENER=${env.RW_GOGGLE_API_TOKEN_SHORTENER} --build-arg NEXT_PUBLIC_RW_MAPBOX_API_TOKEN=${env.RW_MAPBOX_API_TOKEN} --build-arg NEXT_PUBLIC_GLOBAL_FISHING_WATCH_TOKEN=${env.NEXT_PUBLIC_GLOBAL_FISHING_WATCH_TOKEN} --build-arg NEXT_PUBLIC_API_ENV=staging --build-arg NEXT_PUBLIC_ENVS_SHOW=staging,production --build-arg NEXT_PUBLIC_ENVS_EDIT=staging --build-arg NEXT_PUBLIC_AUTH_CALLBACK=https://staging.resourcewatch.org/auth-callback  --build-arg NEXTAUTH_URL=https://staging.resourcewatch.org --build-arg NEXTAUTH_JWT_SECRET=${env.RW_NEXTAUTH_JWT_SECRET} --build-arg NEXT_PUBLIC_GOOGLE_TAG_MANAGER_CONTAINER_ID=GTM-WQBRPVH  -t ${imageTag} .")
          break
        case "preproduction":
