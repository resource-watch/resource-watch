      ...
      switch ("${env.BRANCH_NAME}") {
        case "develop":
          sh("docker -H :2375 build -t ${imageTag}
            ...
            --build-arg RW_GOGGLE_API_TOKEN_SHORTENER=${env.RW_GOGGLE_API_TOKEN_SHORTENER}
            --build-arg RW_MAPBOX_API_TOKEN=${env.RW_MAPBOX_API_TOKEN}
            --build-arg apiEnv=production
            --build-arg apiUrl=https://staging.resourcewatch.org/api
            --build-arg wriApiUrl=https://staging-api.globalforestwatch.org/v1
            --build-arg callbackUrl=https://staging.resourcewatch.org/auth
            --build-arg controlTowerUrl=https://staging-api.globalforestwatch.org
            --build-arg RW_FEATURE_FLAG_AREAS_V2=true
          .")
          break
        case "preproduction":
