...
const getLayerGroup = (state, props) => state.explore.map.layerGroups.find((lg) => lg.dataset === props.dataset.id);
const getLayers = (state, props) => props.layers;

export const isATimelineDataset = createSelector(
  [getLayerGroup],
  (_layerGroup) => !!_layerGroup && _layerGroup.layers.find((l) => l.layerConfig.timeline),
);

export const getTimelineLayerMapbox = (state, props) => props.layers.find((l) => !!l.layerConfig.timeline_config);
...
export const getTimelineLayer = createSelector(
  ...
  (_layerGroup, _isATimeline, _layers) => {
    if (_isATimeline) {
      ...
      if (activeLayer) {
        return activeLayer;
      } if (_layers.length && _layers.length > 0) {
        return _layers[0];
      }
    } else {
      return null;
