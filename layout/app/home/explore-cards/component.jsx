...
export default function ExploreCards() {
  return EXPLORE_CARDS.map((card) => (
    <div key={`explore-card-${card.title}-${card.tag}`} className="column small-12 medium-6">
      <CardStatic
        className={classnames('-alt -clickable', { [card.className]: !!card.className })}
      ...
      >
        <div>
          {card.tag && <h4 className="tag-name">{card.tag}</h4>}
        ...
        <div className="buttons -align-center">
          {card.buttons.map((button) => {
            if (button.loginRequired) {
              return (
                <LoginRequired key={button.path} text={button.loginRequired}>
            ...
            if (button.anchor) {
              return (
                <a
                  href={button.path}
                  key={button.path}
                  className={`c-btn -alt ${button.className}`}
                >
                  {button.text}
                </a>
              );
