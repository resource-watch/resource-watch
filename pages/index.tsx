...
export const getServerSideProps: GetServerSideProps = withRedux(
  withUserServerSide(async ({ store }) => {
    const { getState, dispatch } = store;
    const {
      blog: { latestPosts, spotlightPosts, latestPostsError, spotlightPostsError },
    } = getState();

    // fetches posts from blog when there are no posts
    // to display or when an error happened previously
    if (
      (!latestPosts.length && !spotlightPosts.length) ||
      latestPostsError ||
      spotlightPostsError
    ) {
