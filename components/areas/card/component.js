...
const AreaCard = (props) => {
  const { area, activeAlerts, removeUserArea } = props;
  ...
  useEffect(() => {
    const { geostore: geostoreId, name } = area;
  ...
  const { name, subscription } = area;
  const subscriptionConfirmed = subscription && subscription.confirmed;
...
AreaCard.propTypes = {
  ...
  activeAlerts: PropTypes.array.isRequired,
