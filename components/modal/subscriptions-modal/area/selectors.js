...
export const getActiveArea = (state, props) => state.user.areas.items
  .find(({ id }) => id === props.area);
...
export const getSubscriptionsByArea = createSelector(
  ...
  (_area, _subscriptions) => {
    ...
    return _subscriptions.filter((_subscription) => _subscription.params.area === _area.id);
  },
);
