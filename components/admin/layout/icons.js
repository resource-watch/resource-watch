import React from 'react';

export default class Icons extends React.Component {

  render() {
    return (
      <svg style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }} version="1.1" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <symbol id="icon-arrow-down" viewBox="0 0 24 24">
            <title>arrow-down</title>
            <path d="M23.513 6.825c0 0.3-0.15 0.6-0.3 0.75l-10.5 10.5c-0.45 0.45-1.2 0.45-1.65 0l-10.5-10.5c-0.45-0.45-0.45-1.2 0-1.65s1.2-0.45 1.65 0l9.6 9.9 9.75-9.75c0.45-0.45 1.2-0.45 1.65 0 0.3 0.15 0.3 0.45 0.3 0.75z" />
          </symbol>
          <symbol id="icon-arrow-up" viewBox="0 0 24 24">
            <title>arrow-up</title>
            <path d="M23.513 17.175c0-0.3-0.15-0.6-0.3-0.75l-10.5-10.5c-0.45-0.45-1.2-0.45-1.65 0l-10.5 10.5c-0.45 0.45-0.45 1.2 0 1.65s1.2 0.45 1.65 0l9.6-9.9 9.75 9.75c0.45 0.45 1.2 0.45 1.65 0 0.3-0.15 0.3-0.45 0.3-0.75z" />
          </symbol>
          <symbol id="icon-check" viewBox="0 0 24 24">
            <title>check</title>
            <path d="M21 5q0.43 0 0.715 0.285t0.285 0.715q0 0.422-0.289 0.711l-12 12q-0.289 0.289-0.711 0.289t-0.711-0.289l-6-6q-0.289-0.289-0.289-0.711 0-0.43 0.285-0.715t0.715-0.285q0.422 0 0.711 0.289l5.289 5.297 11.289-11.297q0.289-0.289 0.711-0.289z" />
          </symbol>
          <symbol id="icon-cross" viewBox="0 0 24 24">
            <title>cross</title>
            <path d="M19 4q0.43 0 0.715 0.285t0.285 0.715q0 0.422-0.289 0.711l-6.297 6.289 6.297 6.289q0.289 0.289 0.289 0.711 0 0.43-0.285 0.715t-0.715 0.285q-0.422 0-0.711-0.289l-6.289-6.297-6.289 6.297q-0.289 0.289-0.711 0.289-0.43 0-0.715-0.285t-0.285-0.715q0-0.422 0.289-0.711l6.297-6.289-6.297-6.289q-0.289-0.289-0.289-0.711 0-0.43 0.285-0.715t0.715-0.285q0.422 0 0.711 0.289l6.289 6.297 6.289-6.297q0.289-0.289 0.711-0.289z" />
          </symbol>
          <symbol id="icon-search" viewBox="0 0 24 24">
            <title>search</title>
            <path d="M22.628 20l-4.343-4.457c1.028-1.6 1.715-3.543 1.715-5.486 0-5.486-4.457-10.058-9.943-10.058s-10.057 4.457-10.057 10.058 4.457 10.057 9.943 10.057c2.057 0 3.886-0.572 5.486-1.715l4.342 4.343c0.343 0.343 1.028 0.343 1.372 0l1.372-1.372c0.457-0.343 0.457-1.028 0.114-1.372zM9.943 18.057c-4.343 0-8-3.657-8-8s3.543-8 8-8 8 3.657 8 8-3.543 8-8 8z" />
          </symbol>
          <symbol id="icon-filter" viewBox="0 0 24 24">
            <title>filter</title>
            <path d="M12 0c-6.627 0-12 1.679-12 3.75v2.25l9 9v7.5c0 0.828 1.343 1.5 3 1.5s3-0.672 3-1.5v-7.5l9-9v-2.25c0-2.071-5.373-3.75-12-3.75zM2.212 3.254c0.561-0.32 1.35-0.624 2.28-0.879 2.061-0.564 4.728-0.875 7.508-0.875s5.447 0.311 7.508 0.875c0.93 0.255 1.719 0.559 2.28 0.879 0.37 0.211 0.57 0.39 0.663 0.496-0.093 0.107-0.293 0.285-0.663 0.496-0.561 0.32-1.35 0.624-2.28 0.879-2.061 0.564-4.728 0.875-7.508 0.875s-5.447-0.311-7.508-0.875c-0.93-0.255-1.719-0.559-2.28-0.879-0.37-0.211-0.57-0.39-0.663-0.496 0.093-0.107 0.293-0.285 0.663-0.496z" />
          </symbol>
        </defs>
      </svg>

    );
  }
}
